...
if [ $(uname) = "Darwin" ]; then
  clang++ main.cpp crossprocess.cpp -o xproc -std=c++17 -m32;
elif [ $(uname) = "Linux" ]; then
  g++ main.cpp crossprocess.cpp -o xproc -std=c++17 -static-libgcc -static-libstdc++ -lprocps -lpthread -m32;
elif [ $(uname) = "FreeBSD" ]; then
  clang++ main.cpp crossprocess.cpp -o xproc -std=c++17 -lprocstat -lutil -lc -lpthread -m32;
elif [ $(uname) = "DragonFly" ]; then
  g++ main.cpp crossprocess.cpp -o xproc -std=c++17 -lkvm -lutil -lc -lpthread -m32;
else
  ...
  C:/msys64/msys2_shell.cmd -defterm -mingw32 -no-start -here -lc "g++ main.cpp crossprocess.cpp -o xproc.exe -std=c++17 -static-libgcc -static-libstdc++ -static -lShlwapi -DXPROCESS_WIN32EXE_INCLUDES -m32";
