...
if [ $(uname) = "Darwin" ]; then
  clang++ main.cpp apiprocess/process.cpp -o xproc -std=c++17 -DPROCESS_GUIWINDOW_IMPL -DPROCESS_XQUARTZ_IMPL `pkg-config x11 --cflags --libs` -arch arm64 -arch x86_64;
  ...
  clang++ main.cpp apiprocess/process.cpp -o xproc -std=c++17 -lprocstat -lutil -lc -lpthread -DPROCESS_GUIWINDOW_IMPL `pkg-config x11 --cflags --libs` -m64;
elif [ $(uname) = "DragonFly" ]; then
  g++ main.cpp apiprocess/process.cpp -o xproc -std=c++17 -lkvm -lutil -lc -lpthread -DPROCESS_GUIWINDOW_IMPL `pkg-config x11 --cflags --libs` -m64;
else
